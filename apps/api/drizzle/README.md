...
Kept as a custom migration because `drizzle-kit` does not generate
`CREATE EXTENSION`. Nothing in the current schema needs it — `gen_random_uuid()`
has been core since Postgres 13 — but it is preserved so an existing database's
extension set does not silently change. Do not add more hand-written migrations
alongside it.
...
2. If they match, squash: keep `0000_enable_pgcrypto.sql`, delete the other
   `.sql` files and every `meta/*_snapshot.json`, reset `meta/_journal.json` to
   just the `0000` entry, and run `db:generate --name=baseline`.
...
4. On every database that already ran the old chain, replace the old ledger rows:
   ...
   ```sql
   DELETE FROM drizzle.__drizzle_migrations WHERE created_at > <0000's when>;
   INSERT INTO drizzle.__drizzle_migrations (hash, created_at)
   VALUES ('<sha256 of 0001_baseline.sql>', <0001's "when" from _journal.json>);
   ```
